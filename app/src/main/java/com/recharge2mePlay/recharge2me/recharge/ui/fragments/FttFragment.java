...
public class FttFragment extends Fragment {
        ...
        call.enqueue(new Callback<PlanData>() {
            ...
            public void onResponse(Call<PlanData> call, Response<PlanData> response) {
                ...
                try {
                    ...
                    List<RecTypeFTT> ftt = data.getFTT();
                    ...
                    rv_planFtt.addOnItemTouchListener(new PlanAdapter.planRecyclerTouchListener((GetRechargePlanActivity) requireActivity(),
                        ...
                        public void onPlanClick(View view, int position, View btn) {
                            ...
                            RecTypeFTT sendFtt = ftt.get(position);
    ...
    private void setFttDataOnRecyclerView(List<RecTypeFTT> ftt){

        try {
            PlanAdapter planAdapter_ = new
                    PlanAdapter(null, null, ftt, null, null, (GetRechargePlanActivity) requireActivity());

            rv_planFtt.setAdapter(planAdapter_);
            rv_planFtt.setLayoutManager(new LinearLayoutManager((GetRechargePlanActivity) requireActivity()));
            activity.hideProgressBar();
        }
        catch (Exception e) {
